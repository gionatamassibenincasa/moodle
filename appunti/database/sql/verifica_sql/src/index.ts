...
import { Compito } from "./compito_20191209";
...
let invia = function() {
	...
	let punteggio = artefatti.reduce((acc, sqlStatement, index) => {
		...
		try {
			...
			return (
				(JSON.stringify(res).toLocaleLowerCase() ===
				JSON.stringify(ref).toLocaleLowerCase()
					? Compito.quesiti[index].punteggio
					: 0) + acc
			);
		} catch (e) {
			console.log(e);
	...
	*/
	...
	let consegna: Consegna = new Consegna(
		classe,
		alunno,
		artefatto,
		0 /*punteggio*/
